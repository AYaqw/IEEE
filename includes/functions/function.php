...
function insert_admin($name,$email,$password,$reg_state){
    global $con;
    $stmt = $con->prepare("INSERT INTO admins(name,email,password,role) Value(:username,:email,:password,:reg_state)");
    $stmt->execute(
    array(
        ":username"     => $name,
        ":email"    => $email,
...
function addEvent($eventName,$eventSeason, $eventYear, $eventImg, $eventDescription, $eventLink){
    global $con;
    $stmt=$con->prepare("INSERT INTO
     events(`name`,`season`,`year`,`img`,`description`,`link`)
     VALUES (:eventName,:eventSeason, :eventYear,:eventImg, :eventDescription,:eventLink)" );
    $stmt->execute(array(
        ":eventName"            =>$eventName,
